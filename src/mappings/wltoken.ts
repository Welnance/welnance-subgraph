...
} from '../types/templates/WLToken/WLToken'
import {
  Market,
  Account,
  MintEvent,
...
/* Account supplies assets into market and receives wlTokens in exchange
 *
 * event.mintAmount is the underlying asset
 * event.mintTokens is the amount of wlTokens minted
 * event.minter is the account
 *
 * Notes
 *    Transfer event will always get emitted with this
 *    Mints originate from the wlToken address, not 0x000000, which is typical of ERC-20s
 *    No need to updateMarket(), handleAccrueInterest() ALWAYS runs before this
 *    No need to updateCommonWLTokenStats, handleTransfer() will
 *    No need to update wlTokenBalance, handleTransfer() will
 */
export function handleMint(event: Mint): void {
  ...
  let wlTokenAmount = event.params.mintTokens
    .toBigDecimal()
    .div(wlTokenDecimalsBD)
    .truncate(wlTokenDecimals)
  ...
  mint.wlTokenSymbol = market.symbol
  mint.underlyingAmount = underlyingAmount
  mint.save()
}

/*  Account supplies wlTokens into market and receives underlying asset in exchange
 *
 *  event.redeemAmount is the underlying asset
 *  event.redeemTokens is the wlTokens
...
export function handleRedeem(event: Redeem): void {
  ...
  let wlTokenAmount = event.params.redeemTokens
    .toBigDecimal()
    .div(wlTokenDecimalsBD)
    .truncate(wlTokenDecimals)
  ...
  redeem.wlTokenSymbol = market.symbol
  redeem.underlyingAmount = underlyingAmount
  redeem.save()
}

/* Borrow assets from the protocol. All values either BNB or WBEP20
 *
...
export function handleBorrow(event: Borrow): void {
  ...
  // Update wlTokenStats common for all events, and return the stats to update unique
  // values for each event
  let wlTokenStats = updateCommonWLTokenStats(
    market.id,
    market.symbol,
    accountID,
  ...
  wlTokenStats.storedBorrowBalance = event.params.accountBorrows
    .toBigDecimal()
    .div(exponentToBigDecimal(market.underlyingDecimals))
    .truncate(market.underlyingDecimals)

  wlTokenStats.accountBorrowIndex = market.borrowIndex
  wlTokenStats.totalUnderlyingBorrowed = wlTokenStats.totalUnderlyingBorrowed.plus(
    borrowAmountBD,
  )
  wlTokenStats.save()
...
export function handleRepayBorrow(event: RepayBorrow): void {
  ...
  wlTokenStats.accountBorrowIndex = market.borrowIndex
  wlTokenStats.totalUnderlyingRepaid = wlTokenStats.totalUnderlyingRepaid.plus(
    repayAmountBD,
  )
  wlTokenStats.save()
 ...
 *
 * event.params.borrower - the borrower who is getting liquidated of their wlTokens
 * event.params.wlTokenCollateral - the market ADDRESS of the wltoken being liquidated
 * event.params.liquidator - the liquidator
 * event.params.repayAmount - the amount of underlying to be repaid
 * event.params.seizeTokens - wlTokens seized (transfer event should handle this)
 *
 * Notes
 *    No need to updateMarket(), handleAccrueInterest() ALWAYS runs before this.
 ...
 *    of the wkTokens, which is covered by transfer. Therefore we only
...
export function handleLiquidateBorrow(event: LiquidateBorrow): void {
  ...
  // asset. They seize one of potentially many types of wlToken collateral of
  ...
  let marketwlTokenLiquidated = Market.load(event.params.wlTokenCollateral.toHexString())
  let mintID = event.transaction.hash
    .toHexString()
    .concat('-')
    .concat(event.transactionLogIndex.toString())

  let wlTokenAmount = event.params.seizeTokens
    .toBigDecimal()
    .div(wlTokenDecimalsBD)
    .truncate(wlTokenDecimals)
  ...
  liquidation.amount = wlTokenAmount
  ...
  liquidation.wlTokenSymbol = marketwlTokenLiquidated.symbol
  liquidation.save()
}

/* Transferring of wlTokens
 *
 * event.params.from = sender of wlTokens
 * event.params.to = receiver of wlTokens
 ...
 *      redeemFresh() - i.e. redeeming your wlTokens for underlying asset
 *      mintFresh() - i.e. you are lending underlying assets to create wlTokens
 *      transfer() - i.e. a basic transfer
 *    This function handles all 4 cases. Transfer is emitted alongside the mint, redeem, and seize
 *    events. So for those events, we do not update wlToken balances.
 */
export function handleTransfer(event: Transfer): void {
  ...
  let amountUnderlying = market.exchangeRate.times(
    event.params.amount.toBigDecimal().div(wlTokenDecimalsBD),
  )
  let amountUnderylingTruncated = amountUnderlying.truncate(market.underlyingDecimals)

  // Checking if the tx is FROM the wlToken contract (i.e. this will not run when minting)
  ...
  if (accountFromID != marketID) {
    ...
    // Update wlTokenStats common for all events, and return the stats to update unique
    // values for each event
    let wlTokenStatsFrom = updateCommonWLTokenStats(
      market.id,
      market.symbol,
      accountFromID,
      event.transaction.hash,
    ...
    wlTokenStatsFrom.wlTokenBalance = wlTokenStatsFrom.wlTokenBalance.minus(
      event.params.amount
        .toBigDecimal()
        .div(wlTokenDecimalsBD)
        .truncate(wlTokenDecimals),
    ...
    wlTokenStatsFrom.totalUnderlyingRedeemed = wlTokenStatsFrom.totalUnderlyingRedeemed.plus(
      amountUnderylingTruncated,
    )
    wlTokenStatsFrom.save()
  }

  // Checking if the tx is TO the wlToken contract (i.e. this will not run when redeeming)
  // If so, we ignore it. this leaves an edge case, where someone who accidentally sends
  // wlTokens to a wlToken contract, where it will not get recorded. Right now it would
  ...
  if (accountToID != marketID) {
    ...
    // Update wlTokenStats common for all events, and return the stats to update unique
    // values for each event
    let wlTokenStatsTo = updateCommonWLTokenStats(
      market.id,
      market.symbol,
      accountToID,
      event.transaction.hash,
    ...
    wlTokenStatsTo.wlTokenBalance = wlTokenStatsTo.wlTokenBalance.plus(
      event.params.amount
        .toBigDecimal()
        .div(wlTokenDecimalsBD)
        .truncate(wlTokenDecimals),
    ...
    wlTokenStatsTo.totalUnderlyingSupplied = wlTokenStatsTo.totalUnderlyingSupplied.plus(
      amountUnderylingTruncated,
    )
    wlTokenStatsTo.save()
  }
  ...
  transfer.amount = event.params.amount.toBigDecimal().div(wlTokenDecimalsBD)
  transfer.to = event.params.to
  transfer.from = event.params.from
  transfer.blockNumber = event.block.number.toI32()
  transfer.blockTime = event.block.timestamp.toI32()
  transfer.wlTokenSymbol = market.symbol
  transfer.save()
}
