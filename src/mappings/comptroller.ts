...
import { WLToken } from '../types/templates'
import { Market, Comptroller, Account } from '../types/schema'
import { mantissaFactorBD, updateCommonWLTokenStats, createAccount } from './helpers'
import { createMarket } from './markets'
import { log } from '@graphprotocol/graph-ts'

export function handleMarketListed(event: MarketListed): void {
  // Dynamically index all new listed tokens
  WLToken.create(event.params.wlToken)
  // Create the market for this token, since it's now been listed.
  let market = createMarket(event.params.wlToken.toHexString())
  market.save()
}

export function handleMarketEntered(event: MarketEntered): void {
  let market = Market.load(event.params.wlToken.toHexString())
  ...
  if (market != null) {
    ...
    wlTokenStats.enteredMarket = true
    wlTokenStats.save()
  }
}
